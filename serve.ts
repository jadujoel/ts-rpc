...
const time = (label: string) => Logger.time(`[Server] ${label}`);
...
export function serve({
	...
	Logger = logger;
	log("Server Options", { hostname, port, development, hot });
	const server = Bun.serve({
		...
		websocket: <WebSocketHandler<WebSocketData>>{
			async open(ws): Promise<void> {
				...
				log(
					`[ws] open ${ws.remoteAddress} id: ${ws.data.id} topic: ${topic} subscribers: ${server.subscriberCount(topic)}`,
				);
			...
			message(ws, message): void {
				...
				if (!topic) {
					error(
						`[ws] Failed To Recieve Message For ${ws.data.url} due to no Topic`,
					);
					return;
				}
